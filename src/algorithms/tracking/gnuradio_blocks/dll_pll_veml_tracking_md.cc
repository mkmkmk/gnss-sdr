...
#include "dll_pll_veml_tracking_md.h"
...
{
    printf("********* MK MOD dll_pll_veml_tracking_md ************* *\n");
    ...
    this->set_msg_handler(pmt::mp("telemetry_to_trk"), boost::bind(&dll_pll_veml_tracking_md::msg_handler_telemetry_to_trk, this, _1));
...
void dll_pll_veml_tracking_md::forecast(int noutput_items,
...
void dll_pll_veml_tracking_md::msg_handler_telemetry_to_trk(const pmt::pmt_t &msg)
{
    try
        {
...
dll_pll_veml_tracking_md::~dll_pll_veml_tracking_md()
{
...
bool dll_pll_veml_tracking_md::acquire_secondary()
{
...
bool dll_pll_veml_tracking_md::cn0_and_tracking_lock_status(double coh_integration_time_s)
{
...
void dll_pll_veml_tracking_md::do_correlation_step(const gr_complex *input_samples)
{
...
void dll_pll_veml_tracking_md::run_dll_pll()
{
    // ################## PLL ##########################################################
    // PLL discriminator
    if (d_cloop)
...
void dll_pll_veml_tracking_md::clear_tracking_vars()
{
    std::fill_n(d_correlator_outs, d_n_correlator_taps, gr_complex(0.0, 0.0));
    if (trk_parameters.track_pilot)
        {
...
void dll_pll_veml_tracking_md::update_tracking_vars()
{
    ...
    d_acc_carrier_phase_rad -=
            (d_carrier_phase_step_rad * static_cast<double>(d_current_prn_length_samples) +
...
void dll_pll_veml_tracking_md::save_correlation_results()
{
    if (d_secondary)
        {
...
void dll_pll_veml_tracking_md::log_data()
{
    if (d_dump)
        {
...
int32_t dll_pll_veml_tracking_md::save_matfile()
{
    // READ DUMP FILE
...
void dll_pll_veml_tracking_md::set_channel(uint32_t channel)
{
    gr::thread::scoped_lock l(d_setlock);
    d_channel = channel;
...
void dll_pll_veml_tracking_md::set_gnss_synchro(Gnss_Synchro *p_gnss_synchro)
{
    gr::thread::scoped_lock l(d_setlock);
...
void dll_pll_veml_tracking_md::stop_tracking()
{
    gr::thread::scoped_lock l(d_setlock);
    d_state = 0;
}
                        ...
                        if (d_current_data_symbol == 0)
                            {
                                ...
                                current_synchro_data.Prompt_Q = static_cast<double>(d_P_data_accu.imag());
                                current_synchro_data.Code_phase_samples = d_rem_code_phase_samples;
                                current_synchro_data.Carrier_phase_rads = d_acc_carrier_phase_rad; // tuuu
                                current_synchro_data.Carrier_Doppler_hz = d_carrier_doppler_hz;
                                current_synchro_data.CN0_dB_hz = d_CN0_SNV_dB_Hz;
                                current_synchro_data.correlation_length_ms = d_correlation_length_ms;
                                current_synchro_data.Flag_valid_symbol_output = true;
